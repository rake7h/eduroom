        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['date'] = date('r');
                    ...
                    $q_a= "INSERT INTO files VALUES('', '$filename', '$size', '$type', '$file', '', '','$today')";
                    mysqli_query($database,$q_a) or die(mysqli_error($database));
                    $file_id=mysqli_insert_id($database);
                    $q_b ="INSERT INTO file_relation (`file_file_id`, `file_project_id`, `file_user_id`) VALUES ('$file_id', '$project_id', '$user_id')";
                    mysqli_query($database,$q_b) or die(mysqli_error($database));


//INSERT INTO `files` (`file_id`, `file_name`, `file_size`, `file_tyle`, `file_url`, `file_title`, `file_des`) VALUES (NULL, 'img', '234', 'jpg', 'url', 'title', 'des');
